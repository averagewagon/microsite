...
My objective is to forward all HTTPS requests directed at my router's public
IPv4 address to the microcontroller, and otherwise leave my network as-is.
...
```sh
# Redirect HTTPS (port 80) to HTTPS (port 80) on the microcontroller
...
Since I've set up the rule like this, all HTTP traffic is ignored - my website
is HTTPS-only. I feel somewhat conflicted about this, since restricting insecure
access prohibits many simple browsers, which is somewhat against the spirit of
this project. I waffle on this issue pretty frequently - if someone has a strong
opinion one way or another, feel free to send me an email.

Now, with this configuration done, all HTTPS traffic sent to my router's public
